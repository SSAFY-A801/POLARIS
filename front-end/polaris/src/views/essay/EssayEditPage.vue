<template>
  <div  v-if="essay" class="flex justify-center">
    <div id="app" class="container relative border-2 shadow-gray-300 shadow-lg border-maintheme1 w-full m-16 rounded-lg max-w-4xl p-8">
      ...
      <div class="grid grid-cols-12 gap-8">
        ...
        <input id="title" class="col-span-10 p-2 border-maintheme1 border shadow-md h-10 rounded-lg" v-model="essay.title" type="text">
        <div class="text-lg col-span-2 font-semibold">도서 선택</div>
        <input disabled  class="col-span-10 border shadow-md h-10 p-2 border-maintheme1 rounded-lg" type="text" :placeholder="essay?.bookTitle">
        ...
        <div class="mt-1">
          <label class="relative items-center cursor-pointer">
                  <input @click="toggleOpened" type="checkbox" v-model="essay.isOpened" class="sr-only peer">
                  <div class="after:bg-white  after:border after:rounded-full w-11 h-6 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:h-5 after:w-5 after:transition-all dark:border-gray-600" :class="{ 'bg-gray-200 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full': !essay.isOpened, 'bg-blue-600 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:bg-blue-600':essay.isOpened}"></div>  
          </label> 
        </div>
      </div>
