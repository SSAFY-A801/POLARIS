...
watchEffect(async () => {
  if (selectedSido.value) {
    ...
    await axios.get(`http://i10a801.p.ssafy.io:8082/regcode/gugun?sido=${selectedSido.value.name}`, {withCredentials:true})
...
watchEffect(async () => {
  if (selectedSido.value && selectedGugun.value) {
    await axios.get(`http://i10a801.p.ssafy.io:8082/regcode/dong?sido=${selectedSido.value.name}&gugun=${selectedGugun.value.name}`, {withCredentials:true})
    .then (function(response) {
