<template>
   ...
   <div class="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8">
                             ...
                             <div class="shadow overflow-y-auto h-96 sm:rounded-lg">
                                <table class="min-w-full divide-y divide-gray-200">
                                   ...
                                   <tbody class="bg-white">
                                      <tr v-for="(book, index) in props.books" :key="index" class="index % 2 === 0 ? 'bg-white' : 'bg-gray-50'">
                    ...
                    <div class="flex justify-between">
                     <div class="text-lg font-semibold mt-6">총 구매금액 : {{ totalPrice }} 원</div>
                   </div>
                 </div>
                 <div>
       </div>                 
...
const totalPrice = computed(() => {
  return props.books?.reduce((sum, book) => sum + (book.price || 0), 0);
});
...
const props = defineProps({
  books: Array as () => ChatRoomTradeBooks[]
});
</script>
