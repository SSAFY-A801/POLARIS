...
<div class="container">
    <div class="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8">
      ...
      <button v-if="tradeStatus === 'default' || tradeStatus ==='editing'" @click="confirmClick" class="inline-block rounded bg-maintheme1 px-4 py-2 text-xs font-medium text-white hover:bg-maintheme2">거래확인</button>
      <button v-if="tradeStatus === 'confirmed'" @click="editClick" class="inline-block rounded bg-maintheme1 px-4 py-2 text-xs font-medium text-white hover:bg-maintheme2">거래수정</button>
      <button v-if="tradeStatus === 'confirmed'" @click="completeClick" class="inline-block rounded bg-maintheme1 px-4 py-2 text-xs font-medium text-white hover:bg-maintheme2">거래완료</button>
    </div>
    </div>
                  </div>
                  <div>
      ...
      <div v-else class="text-center py-4">
        ...
        <div class="text-lg mt-5">나의 프로필에서 교환할 도서를 선택하여 도서의 상태를 '교환'으로 변경해주세요!</div>
              ...
              <div class="bg-gray-50 px-4 py-3 sm:px-6 flex align-items justify-end p-4 gap-4 flex-row">
                <button @click="sendSelectedBooks" class="inline-block rounded bg-maintheme1 px-4 py-2 text-xs font-medium text-white hover:bg-maintheme2">담기완료</button>
